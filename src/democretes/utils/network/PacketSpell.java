...
public class PacketSpell extends PacketChoords {
	...
	public PacketSpell(int x, int y, int z) {
		super(x, y, z);
	}
	...
	public IMessage onMessage(PacketChoords message, MessageContext ctx) {
		...
		if(SpellHelper.getNetwork(player).spell != null) {
			...
			network.spell.getActivator().activateSpell(FMLClientHandler.instance().getClient().thePlayer);
			network.spell.getActivator().activateSpell(ctx.getServerHandler().playerEntity);
		}
		return null;
	}
